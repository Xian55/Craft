...
- `deploy_pi.cmd [user@host]` — docker deploy to the always-on Orange Pi (aarch64/Debian): config in `.env` (gitignored; template `.env.example`: PI_HOST/PI_DIR/PI_PORT), rebuilds web, scp's server.js+Dockerfile+web, image `craft-server`, container `craft`, world in `craft_world` volume (`CRAFT_DATA=/data` env picks the data dir). NOTE: .cmd scripts must stay CRLF — LF-only endings make cmd.exe misparse (stray "'m' is not recognized" errors)
