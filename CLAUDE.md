...
- **Determinism gate (critical):** `bun tools\terrain_ref.mjs > ref.txt` vs `..\craft_raylib_build\native\gen_test.exe > c.txt`, `diff --strip-trailing-cr` must be empty. Gate ANY change near `src/gen.c`.
...
- `gen.c` — deterministic terrain noise, bit-identical to legacy JS (ECMA ToInt32 emulation, doubles only, `x*x*sqrt(x)` not pow, `-ffp-contract=off`, never -ffast-math). **Don't touch without the gate.**
